...
1. Werk op `main` zolang dit projectbeleid actief is; maak alleen een aparte branch na expliciete opdracht van Peter.
...
## 6. Actuele stand van de app (25 juli 2026)

Dit is een statusoverzicht van wat inmiddels is gerealiseerd in de applicatie, zodat nieuwe uitvoeringsplannen met de juiste startsituatie beginnen.

### 6.1 Reeds gebouwd

- Eigen loginbasis met e-mail + wachtwoord, sessiebeheer en wachtwoordherstel is aanwezig.
- De route `/taken` gebruikt de goedgekeurde blauw-gele takenweergave.
- In het takenprototype zijn hoofdtaak en subtaak interacties uitgebreid met:
	- hoofdtaak toevoegen;
	- subtaak toevoegen;
	- hoofdtaak bewerken;
	- subtaak bewerken;
	...
	- subtaak verwijderen;
	- deadline-invoer met aparte datum en optionele tijd;
	- bij klikken op dezelfde hoofdtaak/subtaak opnieuw sluiten;
	- bevestiging bij verwijderen;
	- bevestiging opslaan ja/nee bij sluiten of wisselen met niet-opgeslagen wijzigingen.
- De oude groene takenboard-UI-bestanden zijn verwijderd uit de actieve codebasis.

### 6.2 Huidige technische interpretatie

- De takenweergave op `/taken` draait momenteel op lokale prototype-state en voorbeelddata.
- Er bestaat al een server-side taken-domeinlaag en Prisma-migratie voor taken/subtaken/dependencies, maar deze is nog niet de actieve UI-flow van het goedgekeurde takenprototype.

### 6.3 Openstaande implementatielijn

- Koppelen van de goedgekeurde taken-UI aan de server-side takenservices.
- Verdere uitwerking van planningsmotorgedrag op basis van de productregels.
- Afronden van resterende end-to-end kwaliteitscontroles per fase.
